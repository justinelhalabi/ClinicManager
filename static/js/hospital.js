$(document).ready(function () {
    ...
    function addHospital(data) {
        ...
        $.ajax(settings).done(function (response) {
         $.notify("Hospital Added Successfully", {"status":"success"});
...
swal({
 ...
 $.ajax(settings).done(function (response) {
   swal("Deleted!", "Hospital has been deleted.", "success");
    ...
    function updateHospital(data, id) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": "hospital/" + id,
        ...
        $.ajax(settings).done(function (response) {
            $('.modal.in').modal('hide')
            $.notify("Hospital Updated Successfully", {"status":"success"});
            table.destroy();
            $('#datatable4 tbody').empty(); // empty in case the columns change
            getHospital()
        });
        ...
        $.ajax(settings).done(function (response) {
            ...
            $('.btn-edit').one("click", function(e) {
                ...
                $('#myModal').modal().one('shown.bs.modal', function (e) {
                    ...
                    $("#savethehospital").off("click").on("click", function(e) {
                    var instance = $('#detailform').parsley();
                    instance.validate()
                    console.log(instance.isValid())
                    if(instance.isValid()){
                        jsondata = $('#detailform').serializeJSON();
                        updateHospital(jsondata, data.ID)
                        }
    ...
    $("#district_select").html(districtSelect)
...
var districtSelect=""
 function getDistrict() {
        ...
        districtSelect +="<option value="+response[i].ID+">"+response[i].Name+"</option>"
        }
